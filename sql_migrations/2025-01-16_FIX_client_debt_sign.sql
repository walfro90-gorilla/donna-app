-- ============================================================================
-- FIX: Signo de transacción CLIENT_DEBT (debe ser NEGATIVO)
-- ============================================================================
-- Date: 2025-01-16
-- Purpose: 
--   Corregir el signo de la transacción CLIENT_DEBT para que refleje
--   correctamente la deuda del cliente (monto negativo)
-- ============================================================================
-- 🐛 PROBLEMA:
--   La transacción CLIENT_DEBT se estaba creando con signo POSITIVO (+369)
--   cuando debería ser NEGATIVO (-369) para reflejar que el cliente DEBE dinero
--
-- ✅ SOLUCIÓN:
--   Actualizar el RPC mark_order_not_delivered para usar monto negativo
-- ============================================================================
...
BEGIN
  ...
  -- 6.4) Cliente debe dinero (NEGATIVO) ⚠️ CORREGIDO: Ahora es negativo
  INSERT INTO public.account_transactions (
    account_id,
    type,
    amount,
  ...
  RETURN jsonb_build_object(
    ...
    'platform_refund', -(v_restaurant_amount + v_delivery_earning),
    'platform_commission', v_platform_commission,
    'client_debt', -v_total_amount,  -- ✅ Retorna como negativo para indicar deuda
    'failed_attempts', v_new_failed_attempts,
...
BEGIN
  ...
  RAISE NOTICE '  - CLIENT_DEBT ahora usa monto NEGATIVO (-369) en lugar de positivo (+369)';
  RAISE NOTICE '  - Esto refleja correctamente que el cliente DEBE dinero';
  RAISE NOTICE '';
  ...
  RAISE NOTICE '  4. CLIENT_DEBT (-369.00) → Cliente debe dinero (NEGATIVO)';
  RAISE NOTICE '';
  RAISE NOTICE 'SUMA = 283.90 + 29.75 - 313.65 - 369.00 = -369.00';
  RAISE NOTICE '     = Balance pendiente del cliente';
END $$;
...
-- 6. Verifica que en el balance del cliente aparezca NEGATIVO (ej: -$369.00)
-- ============================================================================
