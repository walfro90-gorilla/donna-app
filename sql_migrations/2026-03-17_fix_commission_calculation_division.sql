-- Fix: Commission calculation uses division to extract embedded commission
--
-- BEFORE (wrong): subtotal * rate        → 115 * 0.15 = 17.25
-- AFTER (correct): subtotal * rate / (1 + rate) → 115 * 0.15 / 1.15 = 15.00
--
-- Reason: App prices already include the commission (kitchen_price * 1.15).
-- To extract the commission that is embedded in the price, we must divide by (1 + rate).
...
BEGIN
  IF NEW.status = 'delivered' AND (OLD.status IS DISTINCT FROM 'delivered') THEN
    ...
    -- FIX: Divide to extract the commission already embedded in the app price.
    -- App price = kitchen_price * (1 + rate), so commission = app_price * rate / (1 + rate)
    v_platform_commission := ROUND(COALESCE(NEW.subtotal, NEW.total_amount - COALESCE(NEW.delivery_fee, 0)) * v_commission_rate / (1 + v_commission_rate), 2);
    ...
    IF v_payment_method = 'card' THEN
      ...
      -- 6 transacciones: se cancelan entre sí incluyendo el pago MP (webhook)
      -- =======================================================================
      ...
      IF v_delivery_account_id IS NOT NULL AND COALESCE(NEW.delivery_fee, 0) > 0 THEN
        -- Deuda de la plataforma hacia el repartidor
        INSERT INTO public.account_transactions(account_id, type, amount, order_id, description, metadata)
        VALUES (
    ...
    ELSE
      -- =======================================================================
      -- PAGO EN EFECTIVO: El repartidor cobra y debe entregar parte a plataforma
      -- 5 transacciones que suman exactamente $0 (Balance 0 completo)
      -- CASH_COLLECTED(-total) + DELIVERY_EARNING + RESTAURANT_PAYABLE
      --   + PLATFORM_COMMISSION + PLATFORM_DELIVERY_MARGIN = 0
      -- =======================================================================
      ...
      IF v_delivery_account_id IS NOT NULL AND COALESCE(NEW.delivery_fee, 0) > 0 THEN
        ...
        INSERT INTO public.account_transactions(account_id, type, amount, order_id, description, metadata)
        VALUES (
          v_delivery_account_id, 'CASH_COLLECTED', -NEW.total_amount, NEW.id,
    ...
    RAISE NOTICE '✅ [delivery_v4] order % processed (method=%, net_rest=%, deliv=%, commission=%, delivery_margin=%)',
      NEW.id, v_payment_method, v_restaurant_net, v_delivery_earning, v_platform_commission, v_platform_delivery_margin;
  END IF;
