...
-- Idempotente: solo ejecuta si la columna no existe
...
-- Función trigger: valida que si type='combo' entonces contains tenga productos válidos
...
-- Backfill: poblar contains desde product_combo_items para combos existentes
UPDATE products p
SET contains = (
  SELECT array_agg(pci.product_id ORDER BY pci.created_at)
  FROM product_combos pc
  JOIN product_combo_items pci ON pc.id = pci.combo_id
  WHERE pc.product_id = p.id
)
WHERE p.type = 'combo'
  AND (p.contains IS NULL OR array_length(p.contains, 1) IS NULL)
  AND EXISTS (
