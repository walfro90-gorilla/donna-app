-- ================================================================
-- Fix: process_order_payment_v2 aligned to DATABASE_SCHEMA.sql
-- - Removes ON CONFLICT on non-existent constraint
-- - Adds idempotent guard: skip if transactions already exist for order
-- - Resolves platform accounts by account_type (platform_revenue/payables)
-- - Keeps per-restaurant commission_bps (default 15%)
-- - No creation of users/accounts; uses existing data only
-- ================================================================
...
BEGIN
  ...
  IF NEW.status = 'delivered' AND OLD.status <> 'delivered' THEN
    ...
    -- Idempotent guard: if there are already transactions for this order, skip
    IF EXISTS (SELECT 1 FROM public.account_transactions WHERE order_id = NEW.id) THEN
      RETURN NEW;
    END IF;
    ...
    -- Resolve platform accounts by account_type per schema
    ...
    -- Final guard: required accounts must exist
    ...
    -- Insert transactions (cash vs card flow)
