// Supabase Edge Function: google-maps-proxy
// Secure proxy for Google Places API: Autocomplete, Place Details, Geocode, Reverse Geocode, Address Validation.
...
async function handleAutocomplete(body: any) {
  ...
  if (!API_KEY) return serverError("API_KEY not configured");
  
  const language = (body?.language ?? "es").toString();
  const components = (body?.components ?? "country:mx").toString();
  const sessiontoken = (body?.sessionToken ?? body?.sessiontoken ?? "").toString();
  const types = (body?.types ?? "").toString();
  ...
  console.log("[autocomplete] URL:", url.replace(API_KEY, "***"));
  
  const res = await fetch(url);
  if (!res.ok) {
    console.error("[autocomplete] Upstream error:", res.status, await res.text());
    return serverError(`Upstream error: ${res.status}`);
  }
...
async function handlePlaceDetails(body: any) {
  ...
  console.log("[place_details] URL:", url.replace(API_KEY, "***"));
  
  const res = await fetch(url);
  if (!res.ok) {
    console.error("[place_details] Upstream error:", res.status, await res.text());
    return serverError(`Upstream error: ${res.status}`);
  }
  const data = await res.json();
  ...
  // Extract structured address_components
...
async function handleGeocode(body: any) {
  ...
  console.log("[geocode] URL:", url.replace(API_KEY, "***"));
  
  const res = await fetch(url);
  if (!res.ok) {
    console.error("[geocode] Upstream error:", res.status, await res.text());
    return serverError(`Upstream error: ${res.status}`);
  }
  const data = await res.json();
...
async function handleReverseGeocode(body: any) {
  ...
  const url = `https://maps.googleapis.com/maps/api/geocode/json?${params.toString()}`;
  console.log("[reverse_geocode] URL:", url.replace(API_KEY, "***"));
  
  const res = await fetch(url);
  if (!res.ok) {
    console.error("[reverse_geocode] Upstream error:", res.status, await res.text());
    return serverError(`Upstream error: ${res.status}`);
  }
  const data = await res.json();
  const first = data?.results?.[0];
...
async function handleAddressValidation(body: any) {
  ...
  console.log("[validate_address] URL:", url.replace(API_KEY, "***"));
  ...
  if (!res.ok) {
    console.error("[validate_address] Upstream error:", res.status, await res.text());
    return serverError(`Upstream error: ${res.status}`);
  }
  const data = await res.json();
...
serve(async (req) => {
  ...
  try {
    console.log("[google-maps-proxy] Request received:", req.method, req.url);
    
    if (!API_KEY) {
      console.error("[google-maps-proxy] GOOGLE_MAPS_API_KEY not configured");
      return serverError("GOOGLE_MAPS_API_KEY is not configured in Edge Function secrets");
    }
    ...
    console.log("[google-maps-proxy] Action:", action, "Body:", JSON.stringify(body).substring(0, 200));
