-- =====================================================
-- 87: FIX DELIVERY_AGENT_PROFILES RLS POLICIES
-- =====================================================
...
-- =====================================================
-- PASO 5: Buscar triggers en public.users que creen delivery_agent_profiles
-- =====================================================
...
DECLARE
  trig RECORD;
  func_body TEXT;
  has_suspect_triggers BOOLEAN := FALSE;
BEGIN
  RAISE NOTICE '=== TRIGGERS EN public.users (buscando creación automática) ===';
  ...
  LOOP
    func_body := trig.function_definition;
    
    -- Verificar si el trigger menciona delivery_agent_profiles
    IF func_body ILIKE '%delivery_agent_profiles%' THEN
      has_suspect_triggers := TRUE;
      RAISE WARNING '⚠️ SOSPECHOSO: Trigger % llama a % que menciona delivery_agent_profiles', 
        trig.trigger_name, 
        trig.function_name;
      RAISE NOTICE 'Definición: %', func_body;
    ELSE
      RAISE NOTICE 'Trigger: % | Function: % (no menciona delivery_agent_profiles)', 
        trig.trigger_name,
        trig.function_name;
    END IF;
  END LOOP;
  
  IF NOT has_suspect_triggers THEN
    RAISE NOTICE '✅ No se encontraron triggers sospechosos en public.users';
  END IF;
END $$;
...
DO $$
BEGIN
  RAISE NOTICE '✅ Script 87 completado exitosamente';
  RAISE NOTICE '✅ RLS habilitado en delivery_agent_profiles';
  RAISE NOTICE '✅ Políticas estrictas creadas';
  RAISE NOTICE '⚠️ SIGUIENTE PASO MANUAL: Eliminar registros incorrectos de delivery_agent_profiles desde la consola de Supabase';
