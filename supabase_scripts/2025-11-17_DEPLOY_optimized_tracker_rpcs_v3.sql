-- ============================================================================
-- DEPLOYMENT SCRIPT V3: Funciones RPC Optimizadas para Trackers (CORREGIDO)
-- ============================================================================
-- PROPÓSITO: Desplegar funciones RPC optimizadas que devuelven JSON completo
--            para que ambos trackers (principal y mini) consuman los mismos datos
-- FECHA: 2025-11-17
-- AUTOR: Hologram
-- APEGADO A: DATABASE_SCHEMA.sql (VERIFICADO ESTRICTAMENTE)
-- CAMBIOS V3: CORRECCIÓN QUIRÚRGICA - Removidas columnas inexistentes r.delivery_time y r.delivery_fee
--             Reemplazadas por r.estimated_delivery_time_minutes y r.delivery_radius_km
-- ============================================================================

-- ============================================================================
-- PASO 1: Desplegar get_order_full_details
-- ============================================================================
...
-- ============================================================================
-- PASO 2: Desplegar get_client_active_orders
-- ============================================================================
...
BEGIN
  ...
  WHERE o.user_id = client_id_param
    -- Status activos según DoaOrder model
    AND o.status IN (
...
-- ============================================================================
-- PASO 3: Verificar que ambas funciones existen y funcionan correctamente
-- ============================================================================
...
DECLARE
  func_count INTEGER;
BEGIN
  -- Verificar get_order_full_details
  SELECT COUNT(*) INTO func_count
  FROM pg_proc p
  JOIN pg_namespace n ON p.pronamespace = n.oid
  WHERE n.nspname = 'public'
    AND p.proname = 'get_order_full_details';
  
  IF func_count = 0 THEN
    RAISE EXCEPTION 'ERROR: get_order_full_details no existe';
  ELSE
    RAISE NOTICE '✅ SUCCESS: get_order_full_details creada correctamente';
  END IF;
  
  -- Verificar get_client_active_orders
  SELECT COUNT(*) INTO func_count
  FROM pg_proc p
  JOIN pg_namespace n ON p.pronamespace = n.oid
  WHERE n.nspname = 'public'
    AND p.proname = 'get_client_active_orders';
  
  IF func_count = 0 THEN
    RAISE EXCEPTION 'ERROR: get_client_active_orders no existe';
  ELSE
    RAISE NOTICE '✅ SUCCESS: get_client_active_orders creada correctamente';
  END IF;
  
  RAISE NOTICE '========================================';
  RAISE NOTICE '✅ DEPLOYMENT V3 COMPLETADO EXITOSAMENTE';
  RAISE NOTICE '========================================';
  RAISE NOTICE 'CAMBIOS V3: Columnas corregidas según DATABASE_SCHEMA.sql';
  RAISE NOTICE '- Removido: r.delivery_time (no existe en restaurants)';
  RAISE NOTICE '- Removido: r.delivery_fee (no existe en restaurants)';
  RAISE NOTICE '- Agregado: r.estimated_delivery_time_minutes';
  RAISE NOTICE '- Agregado: r.delivery_radius_km';
  RAISE NOTICE '========================================';
END $$;
...
-- ============================================================================
-- Para ejecutar este script en Supabase SQL Editor:
-- 1. Abrir Supabase Dashboard > SQL Editor
-- 2. Crear "New Query"
-- 3. Copiar y pegar TODO el contenido de este archivo
-- 4. Click en "Run" (o Ctrl+Enter)
-- 5. Verificar que aparecen los mensajes NOTICE de éxito
-- 6. Si NO hay errores, las funciones están listas para usar
-- 7. Los trackers (mini y principal) deberían funcionar automáticamente
-- ============================================================================
