-- =====================================================================
-- Idempotent fix for process_order_payment_v2
-- - Prevents duplicate account_transactions (uses ON CONFLICT DO NOTHING)
-- - Resolves platform accounts using existing account types and email hints
-- - Keeps dynamic restaurant commission (commission_bps) with default 15%
-- - Inserts delivery-related rows only when delivery_agent_account exists
-- - Trigger fires only on transition to delivered
-- =====================================================================
...
BEGIN
  ...
  IF NEW.status = 'delivered' AND OLD.status <> 'delivered' THEN
    ...
    -- Resolve platform revenue account
    ...
    IF platform_revenue_account_id IS NULL THEN
      ...
      WHERE a.account_type IN ('platform','platform_revenue')
        AND u.email ILIKE 'platform+revenue@%'
      ORDER BY a.created_at ASC
      LIMIT 1;
    END IF;

    -- Resolve platform payables account
    ...
    IF platform_payables_account_id IS NULL THEN
      ...
      FROM public.accounts a
      JOIN public.users u ON u.id = a.user_id
      WHERE a.account_type IN ('platform','platform_payables')
        AND u.email ILIKE 'platform+payables@%'
      ORDER BY a.created_at ASC
      LIMIT 1;
    END IF;

    -- Final guard: ensure required accounts exist
    ...
    -- Insert transactions idempotently using the unique constraint on (order_id, account_id, type)
    IF NEW.payment_method = 'cash' THEN
      ...
      -- Delivery earning (only if delivery account exists)
