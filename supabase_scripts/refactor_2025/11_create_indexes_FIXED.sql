...
-- Busqueda geografica (buscar restaurantes cerca) - CORREGIDO: location_lat, location_lon
DROP INDEX IF EXISTS idx_restaurants_location;
CREATE INDEX idx_restaurants_location ON public.restaurants(location_lat, location_lon);
...
-- Filtrar por online (restaurantes activos)
DROP INDEX IF EXISTS idx_restaurants_online;
CREATE INDEX idx_restaurants_online ON public.restaurants(online);
...
-- Busqueda geografica (ordenes cerca de ubicacion) - CORREGIDO: delivery_lat, delivery_lon
...
-- Filtrar productos activos/inactivos - CORREGIDO: is_available
...
-- Buscar settlements por payer_account_id
DROP INDEX IF EXISTS idx_settlements_payer_account_id;
CREATE INDEX idx_settlements_payer_account_id ON public.settlements(payer_account_id);

-- Buscar settlements por receiver_account_id
DROP INDEX IF EXISTS idx_settlements_receiver_account_id;
CREATE INDEX idx_settlements_receiver_account_id ON public.settlements(receiver_account_id);
...
-- Ordenar por fecha de creacion
DROP INDEX IF EXISTS idx_settlements_initiated_at;
CREATE INDEX idx_settlements_initiated_at ON public.settlements(initiated_at DESC);
...
DROP INDEX IF EXISTS idx_reviews_subject_restaurant_id;
CREATE INDEX idx_reviews_subject_restaurant_id ON public.reviews(subject_restaurant_id);

-- Buscar reviews por author
...
-- Ya existe PK en user_id, no necesita indice adicional
