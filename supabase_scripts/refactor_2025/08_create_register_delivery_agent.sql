-- =====================================================
-- FASE 5C: RPC DE REGISTRO DE REPARTIDORES
-- =====================================================
-- Proceso atómico y profesional para registro de repartidores
-- Tiempo estimado: 5 minutos
-- =====================================================

CREATE OR REPLACE FUNCTION public.register_delivery_agent(
  p_email TEXT,
  p_password TEXT,
  p_name TEXT,
  p_phone TEXT,
  p_vehicle_type TEXT DEFAULT 'bicicleta',
  p_emergency_contact_name TEXT DEFAULT NULL,
  p_emergency_contact_phone TEXT DEFAULT NULL
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
DECLARE
  v_user_id UUID;
  v_account_id UUID;
  v_auth_user JSONB;
  v_result JSONB;
  v_error TEXT;
BEGIN
  -- ====================================
  -- VALIDACIONES PREVIAS
  -- ====================================
  
  -- Validar email
  IF p_email IS NULL OR p_email !~ '^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}$' THEN
    RAISE EXCEPTION 'Email inválido: %', p_email;
  END IF;

  -- Validar password
  IF p_password IS NULL OR LENGTH(p_password) < 6 THEN
    RAISE EXCEPTION 'La contraseña debe tener al menos 6 caracteres';
  END IF;

  -- Validar nombre
  IF p_name IS NULL OR LENGTH(TRIM(p_name)) < 3 THEN
    RAISE EXCEPTION 'El nombre debe tener al menos 3 caracteres';
  END IF;

  -- Validar teléfono
  IF p_phone IS NULL OR LENGTH(TRIM(p_phone)) < 8 THEN
    RAISE EXCEPTION 'El teléfono es requerido y debe tener al menos 8 caracteres';
  END IF;

  -- Validar tipo de vehículo
  IF p_vehicle_type NOT IN ('bicicleta', 'motocicleta', 'auto', 'pie', 'otro') THEN
    RAISE EXCEPTION 'Tipo de vehículo inválido: %', p_vehicle_type;
  END IF;

  -- Validar que no exista el email
  IF EXISTS (SELECT 1 FROM auth.users WHERE email = p_email) THEN
    RAISE EXCEPTION 'El email ya está registrado';
  END IF;

  -- Validar que no exista el teléfono
  IF EXISTS (SELECT 1 FROM public.users WHERE phone = p_phone) THEN
    RAISE EXCEPTION 'El teléfono ya está registrado';
  END IF;

  -- ====================================
  -- PASO 1: Crear usuario en auth.users
  -- ====================================
  BEGIN
    v_auth_user := auth.sign_up_v2(
      jsonb_build_object(
        'email', p_email,
        'password', p_password,
        'email_confirm', FALSE,
        'user_metadata', jsonb_build_object(
          'name', p_name,
          'role', 'repartidor'
        )
      )
    );

    v_user_id := (v_auth_user->>'id')::UUID;

    IF v_user_id IS NULL THEN
      RAISE EXCEPTION 'Error al crear usuario en auth.users';
    END IF;

  EXCEPTION
    WHEN OTHERS THEN
      GET STACKED DIAGNOSTICS v_error = MESSAGE_TEXT;
      INSERT INTO debug_logs (scope, message, meta)
      VALUES ('REGISTER_DELIVERY_ERROR', 'Error en auth.sign_up_v2', 
              jsonb_build_object('error', v_error, 'email', p_email));
      RAISE EXCEPTION 'Error al crear usuario: %', v_error;
  END;

  -- ====================================
  -- PASO 2: Crear registro en public.users
  -- ====================================
  INSERT INTO public.users (
    id,
    email,
    name,
    phone,
    role,
    email_confirm,
    created_at,
    updated_at
  ) VALUES (
    v_user_id,
    p_email,
    p_name,
    p_phone,
    'repartidor',
    FALSE,
    NOW(),
    NOW()
  );

  -- ====================================
  -- PASO 3: Crear perfil de repartidor
  -- ====================================
  INSERT INTO public.delivery_agent_profiles (
    user_id,
    vehicle_type,
    emergency_contact_name,
    emergency_contact_phone,
    status, -- online/offline
    account_state, -- pending/approved
    onboarding_completed,
    created_at,
    updated_at
  ) VALUES (
    v_user_id,
    p_vehicle_type,
    p_emergency_contact_name,
    p_emergency_contact_phone,
    'offline', -- Inicia offline (enum delivery_agent_status)
    'pending', -- Debe ser aprobado por admin (enum delivery_agent_account_state)
    FALSE,
    NOW(),
    NOW()
  );

  -- ====================================
  -- PASO 4: Crear cuenta financiera
  -- ====================================
  INSERT INTO public.accounts (
    id,
    user_id,
    account_type,
    balance,
    created_at,
    updated_at
  ) VALUES (
    gen_random_uuid(),
    v_user_id,
    'delivery_agent',
    0.00,
    NOW(),
    NOW()
  )
  RETURNING id INTO v_account_id;

  -- ====================================
  -- PASO 5: Crear preferencias de usuario
  -- ====================================
  INSERT INTO public.user_preferences (
    user_id,
    has_seen_onboarding,
    has_seen_delivery_welcome,
    created_at,
    updated_at
  ) VALUES (
    v_user_id,
    FALSE,
    FALSE,
    NOW(),
    NOW()
  );

  -- ====================================
  -- PASO 6: Crear notificación para admin
  -- ====================================
  INSERT INTO public.admin_notifications (
    target_role,
    category,
    entity_type,
    entity_id,
    title,
    message,
    metadata
  ) VALUES (
    'admin',
    'registration',
    'delivery_agent',
    v_user_id,
    'Nuevo repartidor registrado',
    p_name || ' se ha registrado como repartidor y está pendiente de aprobación.',
    jsonb_build_object(
      'user_id', v_user_id,
      'name', p_name,
      'email', p_email,
      'phone', p_phone,
      'vehicle_type', p_vehicle_type
    )
  );

  -- ====================================
  -- PASO 7: Log de éxito
  -- ====================================
  INSERT INTO debug_logs (scope, message, meta)
  VALUES (
    'REGISTER_DELIVERY_SUCCESS',
    'Repartidor registrado exitosamente',
    jsonb_build_object(
      'user_id', v_user_id,
      'account_id', v_account_id,
      'email', p_email,
      'name', p_name,
      'vehicle_type', p_vehicle_type,
      'timestamp', NOW()
    )
  );

  -- ====================================
  -- PASO 8: Construir respuesta
  -- ====================================
  v_result := jsonb_build_object(
    'success', TRUE,
    'user_id', v_user_id,
    'account_id', v_account_id,
    'email', p_email,
    'name', p_name,
    'role', 'repartidor',
    'status', 'offline',
    'account_state', 'pending',
    'vehicle_type', p_vehicle_type,
    'message', 'Repartidor registrado exitosamente. Tu cuenta está pendiente de aprobación por el administrador.'
  );

  RETURN v_result;

EXCEPTION
  WHEN OTHERS THEN
    GET STACKED DIAGNOSTICS v_error = MESSAGE_TEXT;
    
    -- Log de error
    INSERT INTO debug_logs (scope, message, meta)
    VALUES (
      'REGISTER_DELIVERY_ERROR',
      'Error en registro de repartidor',
      jsonb_build_object(
        'error', v_error,
        'email', p_email,
        'name', p_name,
        'timestamp', NOW()
      )
    );

    -- Retornar error
    RETURN jsonb_build_object(
      'success', FALSE,
      'error', v_error
    );
END;
$$;

-- Permisos
GRANT EXECUTE ON FUNCTION public.register_delivery_agent TO anon, authenticated;

-- Comentario
COMMENT ON FUNCTION public.register_delivery_agent IS 
'Registra un nuevo repartidor en el sistema de manera atómica.
Crea usuario, perfil de delivery, cuenta financiera, preferencias y notificación admin.
El repartidor queda en estado "pending" hasta aprobación de admin.
Retorna JSONB con success: true/false y datos completos o error.';

-- Test rápido (comentar después de verificar)
-- SELECT public.register_delivery_agent(
--   'test_delivery@example.com',
--   'password123',
--   'Carlos Mendez',
--   '+1234567890',
--   'motocicleta',
--   'María Mendez',
--   '+0987654321'
-- );
