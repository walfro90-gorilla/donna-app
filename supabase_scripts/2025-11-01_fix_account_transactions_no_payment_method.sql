-- ============================================================================
-- FIX: Eliminar columna inexistente payment_method de account_transactions
-- ============================================================================
-- PROBLEMA: El trigger intentaba INSERT con payment_method en account_transactions,
--           pero esa tabla NO tiene esa columna (solo: id, account_id, type, 
--           amount, order_id, settlement_id, description, metadata, created_at).
--           
-- SOLUCIÓN: Remover payment_method de todos los INSERT. El método de pago
--           ya está en orders.payment_method y se puede leer de ahí si se necesita.
-- ============================================================================
...
-- Paso 2: Crear función CORRECTA usando schema real
...
BEGIN
    ...
    IF NEW.status = 'delivered' AND (OLD.status IS NULL OR OLD.status != 'delivered') THEN
        ...
        IF v_platform_revenue_account_id IS NULL THEN
            RAISE EXCEPTION 'Platform revenue account not found';
        END IF;
        
        IF v_platform_payables_account_id IS NULL THEN
            RAISE EXCEPTION 'Platform payables account not found';
        END IF;
        ...
        -- Leer payment_method desde orders (NO desde payments)
