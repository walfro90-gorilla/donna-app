-- =============================================================
-- Recreate RPC: register_delivery_agent_v2 (atomic, SECURITY DEFINER)
-- Purpose: Full registration flow for delivery agents, mirroring register_restaurant_v2
-- Notes:
--   - Idempotent upserts to users (via ensure_user_profile_public), delivery_agent_profiles,
--     accounts (account_type='delivery_agent'), and user_preferences.
--   - Uses only columns that exist in delivery_agent_profiles (no lat/lon/address_structured here).
--   - Safe to run multiple times; re-grants EXECUTE with exact signature.
--   - Does NOT change table schemas or constraints (surgical).
-- =============================================================
...
BEGIN
  ...
  --    Note: delivery_agent_profiles does NOT contain lat/lon/address_structured; those live in users.
  ...
  -- 5) Normalize users.role to delivery_agent if empty or client variants
...
-- Re-grant execute with the exact signature so authenticated callers can run it
...
COMMENT ON FUNCTION public.register_delivery_agent_v2(
  uuid, text, text, text, text, double precision, double precision, jsonb,
  text, text, text, text, text, text, text, text, text, text, text, text, text
) IS 'Atomic delivery agent registration: ensure user profile, upsert delivery_agent_profiles, create accounts entry, ensure user_preferences, and set users.role=delivery_agent.';
